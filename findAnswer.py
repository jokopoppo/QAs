...
for i in range(real_answer.__len__()):
    ...
    if real_answer[i] != doc_id[i][1][0]:
        string += str(i) + ' ' + str(real_answer[i]) + ' ' + str(doc_id[i][1][0]) + ' ' + str(possible_answer[i]) + ' ' + str(all_rs[i]) +'\n'
        miss+=1
print(miss)
string += str(miss)
